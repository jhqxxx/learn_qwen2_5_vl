...
impl Qwen2_5_VLForConditionalGeneration {
    pub fn new(cfg: Qwen2_5VLConfig, vb: VarBuilder) -> Result<Self> {
        let visual =
            Qwen2_5_VisionTransformerPretrainedModel::new(&cfg.vision_config, vb.pp("visual"))?;
    ...
    pub fn get_rope_index(
        ...
        if image_grid_thw.is_some() || video_grid_thw.is_some() {
            ...
            let mut attention_mask_;
            if attention_mask.is_none() {
                attention_mask_ = Tensor::ones_like(&total_input_ids)?;
            } else {
                attention_mask_ = attention_mask.unwrap().clone();
            }
            ...
            for i in 0..total_input_ids.dim(0)? {
                ...
                let mask_i = attention_mask_.i(i)?;
                ...
                let vision_tokens = input_ids_i.gather(&vision_indices, 0)?.to_vec1::<u32>()?;
                let vision_indices_vec = vision_indices.to_vec1::<u32>()?;
            ...
            if attention_mask.is_some() {
                let mut position_ids = attention_mask
                    .unwrap()
                ...
                for i in 0..position_ids.dim(0)? {
                    ...
                    let mask_i = attention_mask.unwrap().i(i)?;
                    if mask_i.sum_all()?.to_scalar::<u32>()? != mask_i.dim(0)? as u32 {
                ...
                let mut mrope_position_deltas = position_ids
                    ...
                    .broadcast_sub(&Tensor::new(
                        vec![attention_mask.unwrap().dim(D::Minus1)? as u32 - 1],
                        input_ids.device(),
                    )?)?
                    .contiguous()?;
                if mrope_position_deltas.rank() == 1 {
    ...
    pub fn forward(
        ...
        if pixel_values.is_some() && image_grid_thw.is_some() {
            ...
            let mask = get_equal_mask(&input_ids, self.cfg.image_token_id as u32)?;

            let n_image_tokens = mask.sum_all()?.to_scalar::<u32>()?;
            ...
            inputs_embeds = masked_scatter_dim0(&inputs_embeds, &image_embed, &mask)?;
        }
        if pixel_values_video.is_some() && video_grid_thw.is_some() {
            ...
            let mask = get_equal_mask(&input_ids, self.cfg.video_token_id as u32)?;
            let n_video_tokens = mask.sum_all()?.to_scalar::<u32>()?;
            ...
            inputs_embeds = masked_scatter_dim0(&inputs_embeds, &video_embed, &mask)?;
        }
        let mut position_ids;
        let mut rope_deltas;
        // println!("cache_position: {:?}", cache_position);
        // println!("cache_position: {}", cache_position.unwrap().as_ref());
        if (cache_position.is_some() && cache_position.unwrap().i(0)?.to_scalar::<u32>()? == 0)
            || self.rope_deltas.is_none()
        {
            (position_ids, rope_deltas) =
                self.get_rope_index(input_ids, image_grid_thw, video_grid_thw, Some(attention_mask))?;
            ...
            let delta = if cache_position.is_some() {
                cache_position
                    .unwrap()
                    .i(0)?.to_dtype(self.rope_deltas.as_ref().unwrap().dtype())?
                    .broadcast_add(&self.rope_deltas.as_ref().unwrap())?
                    .contiguous()?.to_dtype(candle_core::DType::U32)?
            } else {
        ...
        let outputs = self
            .model
            .forward(&inputs_embeds, seqlen_offset, Some(&position_ids), attention_mask, cache_position)?;
